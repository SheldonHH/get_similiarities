...
folders = ["c0","c1","c2","c3"]
for fd in folders:
    directory = os.fsencode("/data/get_similiarities/libquantumida/"+fd)
...
for fd in folders:     # 对每一个folder遍历
    ...
    for fuc in funcs:
        ...
        oj = findf(fuc,"/data/get_similiarities/libquantumida/"+fd)
    ...
    with open(r'/data/get_similiarities/libquantumida/fold_dict'+str(outer_index)+'.txt', 'w') as fp:
...
while cs_cont < 3:
    ...
    with open(r'/data/get_similiarities/libquantumida/cs_'+str(cs_cont)+'cs_cont.txt', 'w') as fp:
        for item in cs_arr:
            # write each item on a new line
            fp.write("%s\n" % item)
        print('Done')

    with open(r'/data/get_similiarities/libquantumida/arr_'+str(cs_cont)+'i_arr.txt', 'w') as fp:
